...
class EmployeurSlidesActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Configurez les indicateurs de tabulation
        val tabMediator = TabLayoutMediator(tabLayout, viewPager) { _, _ ->
            // Rien à faire ici
        }.attach()

        // Créez une instance de la classe Drawable pour chaque état de l'indicateur
        ...
        // Configurez les indicateurs pour chaque tabulation
        for (i in 0 until tabLayout.tabCount) {
            val tab = tabLayout.getTabAt(i)
            tab?.icon = if (i == 0) indicateurSelectionne else indicateurNonSelectionne
        }

        // Créez un callback pour gérer les changements de page
